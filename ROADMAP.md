...
| 43h | CIK-level type differentiation | High | **Depends on Phase 4** (needs entity_id on holdings). Currently every subsidiary CIK inherits parent's single manager_type (BlackRock = passive for ALL 30+ subsidiaries). Need per-entity_id type: BlackRock Fund Advisors = passive, BlackRock Capital Investment Advisors = active. Sources: ADV strategy_inferred per CRD→CIK, fund_classification rollup to CIK, subsidiary name keywords. Sequence: Phase 4 → 43h → 43d. |
