...
| INF5 | Fix `_verify_adv_relationship()` parent-not-in-ADV silent skip | Not started | Discovered 2026-04-10 during L5 batch 1 audit. The Tier 1 (AUM sanity) check in `entity_sync._verify_adv_relationship()` only fails when both parent and child have ADV rows AND the ratio is bad. When the parent entity has NO `adv_managers` row at all (because it was created from the legacy managers feed and never registered in ADV), Tier 1 silently skips and the function returns PASS — even for clearly wrong relationships. Test result: 5 of 6 known-bad ADV mis-matches in this batch (AGF→Ally Bridge, Cairn→CI Investments, Americana→&Partners, Edgewood→GoodHaven, Huntington→Linscomb) returned PASS instead of failing. Fix: when `_lookup_adv_row(parent)` returns None, return `(False, 'low', 'tier1_parent_not_in_adv_managers')`. The parent of an ADV_SCHEDULE_A relationship should always be a registered adviser; absence is itself a verification failure. |
...
| INF9 | Persist 2026-04-10 session fixes to entity_overrides_persistent | **Required before next `build_entities.py --reset`** | _Staging workflow required (INF1)._ All 39 manual fixes applied during the 2026-04-10 session live as direct rows in `entity_rollup_history` and `entity_classification_history`, NOT in `entity_overrides_persistent`. A future `build_entities.py --reset --refresh-reference-tables` would re-derive the entity layer from scratch and **wipe** every fix from this session, re-introducing the original ADV mis-matches and classification errors. **Do not run `build_entities.py --reset` in production until this item is complete.** Required action: write 39 rows to `entity_overrides_persistent` with `source`, `reason`, and `analyst` fields populated, covering: (a) 28 DM13 ADV/parent_bridge relationship deletions from L5 batches 1-6, (b) 7 DM12 narrow routing fixes (Securian/Sterling, HC Capital ×7 sub-advisers, CRI ×5 sub-advisers), (c) 4 classification changes from Section 3 (market_maker reclassifications, singleton merges, activist flag fixes, SC US → venture_capital). Each row should reference the originating session commit hash for traceability. After persistence, validate by running `--reset` against staging and confirming `diff_staging.py` returns 0 changes. |
