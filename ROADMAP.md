...
| 43c | Review LOW + MEDIUM confidence classifications 1-by-1 | In Progress | L3 entity_type audit: 11 of 13 categories audited. Fixes applied: passive 59→49 (9 misclassified wealth/quant/strategic), quantitative 68→43 (market makers + multi-strat → hedge_fund), hedge_fund cleaned (24 traditional active managers + wealth advisors removed), PE 168→77 (wealth advisors + VC removed), strategic cleaned (J.Stern $275B → active, VC/PE moved), wealth_management cleaned (HRT→quant, DeepCurrents→HF), SWF fixed (Nordic pensions→pension, Norges Bank→SWF). Remaining: mixed (1,423 CIKs) and active (4,719 CIKs). |
