...
| 43g | Drop is_actively_managed — migrate to fund_strategy | Medium | is_actively_managed (binary true/false) is redundant now that fund_strategy (active/passive/mixed) exists on both fund_universe and fund_holdings_v2. 14 query references in queries.py use `fu.is_actively_managed = true` — migrate to `fu.fund_strategy != 'passive'`. Then drop is_actively_managed column. Do after full classification chain cleanup. |
