...
| # | Item | Priority | Notes |
|---|------|----------|-------|
| INF2 | Monthly maintenance checklist | Recurring | First of each month: `python3 scripts/backup_db.py` (full DuckDB EXPORT DATABASE), `python3 scripts/validate_entities.py` (production health check, expect at most 1 non-structural FAIL — `wellington_sub_advisory` is currently expected and tracked separately), review `manual_routing_review` gate output for overdue manual routings, `python3 scripts/diff_staging.py` to confirm staging matches production at month boundary. See MAINTENANCE.md. |
