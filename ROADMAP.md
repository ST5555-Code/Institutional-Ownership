...
| # | Item | Priority | Notes |
|---|------|----------|-------|
| INF3 | Fix wellington_sub_advisory validation gate | Not started | Scope check to economic_control_v1 only. Under decision_maker_v1 routing fund series to Wellington is correct behavior (DM1 builds these routings deliberately). Gate currently reports FAIL incorrectly when run against production after DM1 — surfaces 122 fund_rollups_to_wellington and 78 "without ncen adviser role" issues that are all legitimate DM1 routings. Fix in next entity fix session. Until fixed, `validate_entities.py` exits rc=1 (non-structural FAIL) on prod, which `promote_staging.py` correctly classifies as non-blocking. |
