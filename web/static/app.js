...
function switchTab(tabId) {
    ...
    if (tabId !== 'two-co-overlap') {
        ...
        if (_tcoResults) { _tcoResults.classList.remove('hidden'); _tcoResults.style.display = ''; }
        const _tcoActionBar = document.querySelector('.action-bar');
        if (_tcoActionBar) { _tcoActionBar.classList.remove('hidden'); _tcoActionBar.style.display = ''; }
    }
    ...
    if (tabId === 'fund-portfolio') {
        ...
        // React POC scaffold — show the tco panel, hide results-area and
        // action-bar, then mount the React app via window.tcoActivate().
        // The React bundle is served from /static/dist/tco-bundle.js.
        const tcoPanelShow = document.getElementById('two-co-overlap-tab');
        if (tcoPanelShow) tcoPanelShow.classList.remove('hidden');
