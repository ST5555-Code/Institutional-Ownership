...
| int-19 | INF38 BLOCK-FLOAT-HISTORY — true pct_of_so tier | — | INF38 | REWRITE_PCT_OF_SO_PERIOD_ACCURACY §14.10 | DEFERRED | `scripts/fetch_sec_float.py` (new), `scripts/enrich_holdings.py` Pass B | pct-of-so live (done) | Phase 2 | Requires 10-K/13D/Forms 3-5 ingestion. |
...
| obs-13 | DIAG-23 Register %FLOAT stale dist bundle (INF42 hygiene) | — | INF42 | POST_MERGE_REGRESSIONS_DIAGNOSTIC §1 | CLOSED (PR #65) | `web/react-app/dist/` (rebuild artifact), `.gitignore` | None | verify | Verification PASS: React source + rebuilt dist bundle (2026-04-19 15:26 EDT) both free of `pct_of_so`; ff1ff71 touched CI fixtures only. Evidence: `docs/findings/obs-13-verify-findings.md`. INF42 (derived-artifact hygiene CI gate) remains a standing gap. |
...
- **INF38 BLOCK-FLOAT-HISTORY** (int-19): True period-accurate `pct_of_so` from 10-K Item 5 + Schedule 13D/G + Forms 3/4/5 insider holdings. Additional tier above `pct_of_so`.
