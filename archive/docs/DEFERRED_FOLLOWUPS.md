...
| INF38 | BLOCK-FLOAT-HISTORY — true float-adjusted `pct_of_so` denominator | `docs/findings/2026-04-19-rewrite-pct-of-so-period-accuracy.md §14.10`; `ROADMAP.md §Open items` | Low-Medium | **UNBLOCKED post-Phase-2.** First new `SourcePipeline` subclass has landed (`load_13f_v2.py`). Still needs a new float-history data source before the tier can be implemented in `enrich_holdings.py` Pass B. |
...
| `docs/pipeline_violations.md`: close `pct_of_so` violation entries with Phase 1b/1c/4b commit citations | `docs/pipeline_violations.md` | Next batched doc-update |
