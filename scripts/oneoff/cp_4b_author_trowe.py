#!/usr/bin/env python3
"""cp_4b_author_trowe.py — CP-4b-author-trowe: single-brand AUTHOR_NEW_BRIDGE.

Inserts one entity_relationships row bridging T. Rowe Price brand
eid=17924 (T. Rowe Price Associates, Inc.) to filer eid=3616
(PRICE T ROWE ASSOCIATES INC /MD/) via wholly_owned/control.

Pairing source: cp-4b-discovery rank 5 (PR #260, HIGH confidence)
via single CRD chain: 105496 -> cik:80255. Fund AUM bridged: ~$1.11T.

Standard CP-4b BRIDGE shape per PR #258 (CP-4b-blackrock) precedent.
Pure new-row INSERT. No fund_holdings_v2 re-point, no SCD closure on
brand eid, no recompute pipelines. peer_rotation_flows expected delta 0.
Brand eid stays alive; bridge metadata only.

Direct prod write per docs/decisions/inst_eid_bridge_decisions.md
(staging-workflow note: CP-4a + CP-4b-blackrock precedent).

Hard guards (--confirm):
  - Brand and filer entity rows present.
  - No existing open bridge between (3616, 17924).
  - Filer 3616 hv2 presence non-zero.
...
MANIFEST_CSV = BASE_DIR / "data" / "working" / "cp-4b-author-trowe-manifest.csv"
...
PAIRING_SOURCE = "cp-4b-discovery_rank5"
CONFIDENCE = "HIGH"
CRD_CHAIN = "105496->cik:80255"


def _build_source(pair_no: int) -> str:
    return (
        f"CP-4b-author-trowe|pair={pair_no}|"
        f"pairing_source={PAIRING_SOURCE}|confidence={CONFIDENCE}|"
        f"crd_chain={CRD_CHAIN}"
    )
...
PAIR = BridgePair(pair_no=PAIR_NO, brand_eid=BRAND_EID, brand_label="T. Rowe Price Associates, Inc.")
...
def main() -> int:
    ...
    try:
        ...
        # Belt-and-suspenders: prepared_relationship_id must not already exist
        exists = int(
            con.execute(
