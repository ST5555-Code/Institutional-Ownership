#!/usr/bin/env python3
"""cp_4b_author_first_trust.py — CP-4b-author-first-trust: single-brand AUTHOR_NEW_BRIDGE.

Inserts one entity_relationships row bridging First Trust brand
eid=8 (First Trust) to filer eid=136 (FIRST TRUST ADVISORS LP)
via wholly_owned/control.

Pairing source: cp-4b-blocker2-corroboration-probe Bucket B
(X1+X2 concordant), MEDIUM confidence, public-record verified
per docs/findings/cp-4b-blocker2-corroboration-probe.md §7.
Fund AUM bridged: ~$232.7B.

Standard CP-4b BRIDGE shape per PR #267 (cp-4b-author-trowe) and
PR #258 (cp-4b-blackrock) precedent. Pure new-row INSERT. No
fund_holdings_v2 re-point, no SCD closure on brand eid, no
recompute pipelines. peer_rotation_flows expected delta 0.
...
Hard guards (--confirm):
  ...
  - No existing open bridge between (136, 8).
  - Filer 136 hv2 presence non-zero.
...
MANIFEST_CSV = BASE_DIR / "data" / "working" / "cp-4b-author-first-trust-manifest.csv"
...
PAIRING_SOURCE = "cp-4b-corroboration-probe-bucket-B"
CONFIDENCE = "MEDIUM"
SIGNALS = "X1+X2"
...
def _build_source(pair_no: int) -> str:
    return (
        f"CP-4b-author-first-trust|pair={pair_no}|"
...
PAIR = BridgePair(pair_no=PAIR_NO, brand_eid=BRAND_EID, brand_label="First Trust")
