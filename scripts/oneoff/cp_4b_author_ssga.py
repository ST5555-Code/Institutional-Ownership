#!/usr/bin/env python3
"""cp_4b_author_ssga.py — CP-4b-author-ssga: single-brand AUTHOR_NEW_BRIDGE.

Inserts one entity_relationships row bridging State Street / SSGA brand
eid=3 to filer eid=7984 (State Street Corp) via wholly_owned/control.
...
Fund AUM bridged: ~$301.9B.

Standard CP-4b BRIDGE shape per PR #267 (cp-4b-author-trowe), PR #269
(cp-4b-author-first-trust), and PR #270 (cp-4b-author-fmr) precedent.
Pure new-row INSERT. No fund_holdings_v2 re-point, no SCD closure on
brand eid, no recompute pipelines. peer_rotation_flows expected delta
0. Brand eid stays alive; bridge metadata only. Closes the cp-4b
carve-out arc.

Direct prod write per docs/decisions/inst_eid_bridge_decisions.md
(staging-workflow note: CP-4a + CP-4b-blackrock + CP-4b-trowe +
CP-4b-first-trust + CP-4b-fmr precedent).

Hard guards (--confirm):
  - Brand and filer entity rows present, both entity_type='institution'.
  - No existing open bridge between (7984, 3).
  - Filer 7984 hv2 presence non-zero.
...
MANIFEST_CSV = BASE_DIR / "data" / "working" / "cp-4b-author-ssga-manifest.csv"
...
def _build_source(pair_no: int) -> str:
    return (
        f"CP-4b-author-ssga|pair={pair_no}|"
...
PAIR = BridgePair(pair_no=PAIR_NO, brand_eid=BRAND_EID, brand_label="State Street / SSGA")
