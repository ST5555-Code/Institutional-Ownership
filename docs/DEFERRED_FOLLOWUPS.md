...
| int-22 | Prod `is_latest` inversion on `holdings_v2` 2025Q4 — rollback AWAITING PROD EXECUTION | `docs/findings/int-22-p0-findings.md`; `scripts/rollback_run.py` | High | **NEW (int-22).** Staging-proven rollback for run_id `13f_holdings_quarter=2025Q4_20260422_200854` (manifest_id=78902). Terminal-only prod execution — see §8 of the findings doc. Closes on successful prod rollback + post-state verification. |
| int-23 | `load_13f_v2.py` idempotency + enrichment sequencing | `docs/findings/int-22-p0-findings.md §10`; `scripts/load_13f_v2.py:515`; `scripts/pipeline/base.py:436` | Medium | **NEW (int-22).** Re-running the 13F loader against an already-loaded quarter corrupts `is_latest` because `ticker` is populated by a separate downstream enrichment step the loader does not coordinate with. Two possible fixes to flag, not choose: (a) `_promote_append_is_latest` refuses the flip when the new population has NULL on a column the displaced population had populated; (b) enrichment becomes atomic with load (same `SourcePipeline` promote step). Do NOT fix in int-22. |
