...
2. **Retention of the 2 holdings_v2 pre-apply snapshots.** Both are 12.27M-row full copies taken immediately before large data remediations (manager_type legacy audit, pct_of_so apply). Are they still needed as revert references during the B2.5 / B3 / Q1-Q2 gate window, or has their role been superseded by `docs/findings/2026-04-19-rewrite-pct-of-so-period-accuracy.md` + migration history in `data_layers.md` Appendix A? These 2 snapshots account for ~80% of total snapshot rows (24.5M of 30.47M).
