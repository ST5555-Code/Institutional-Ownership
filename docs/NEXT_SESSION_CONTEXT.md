...
11c-d. **INF23 — entity fragmentation cleanup (Morningstar, DFA, + CIK backfill).** Surfaced during DM15 scope check. Two fragmented clusters where the same firm has two entity_ids, one with CIK (canonical) and one without: **Morningstar Investment Management** canonical eid=10513 (CIK `0001673385`) vs fragmented eid=19596 (no CIK); **Dimensional Fund Advisors** canonical eid=5026 (CIK `0000354204`) vs fragmented eid=18096 (no CIK). DM1 phase-1 N-CEN routings wrote to the fragmented eids; DM15 Layer 1 (item 11c above) routes to the canonical eids — creating an intra-cluster eid mix that should be resolved by merging fragmented→canonical (INF4/INF6/INF7/INF8 pattern). Additionally, two target entities have no CIK at all: Smith Capital Investors eid=18899 and Milliman Financial Risk Management eid=18304. CIK backfill needed on both via ADV CRD→CIK lookup to close the 8 NULL-CIK DM15 replay gap (Smith ×2 + Milliman ×6). Single dedicated INF23 session: merge fragmented eids + backfill CIKs + re-route existing DM routings to canonical. After INF23 lands, the 8 currently-NULL-CIK DM15 overrides can be re-written against canonical CIKs.
