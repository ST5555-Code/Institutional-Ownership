...
- **`ingestion_impacts.promoted_at` for run `nport_20260415_060422_352131` is reconstructed.** During the 2026-04-16 part-2 session a scoped re-promote for the 244 newly-resolvable series was run after the Tier A+B resolver created 279 new entities in prod. An earlier over-scoped attempt was killed mid-loop; `promote_nport.py`'s `_mirror_manifest_and_impacts` step had already wiped the original 2026-04-15 `promote_status='promoted'` rows for the run. A SQL reconciliation then marked impacts as `promoted` for all `(series_id, report_month)` tuples where `fund_holdings_v2` has matching rows, using `MAX(loaded_at)` per-tuple as a best-effort `promoted_at` proxy. Holdings data itself is correct — the caveat is strictly audit-trail: if any future query reads `promoted_at` for that run, the timestamps reflect load time, not original promote time. 522 tuples remain genuinely pending (deferred synthetics with no prod rows).
...
11b. **DM14 Layer 1 — intra-firm sub-adviser collapse** — **STAGED 2026-04-16** (awaiting review before promote). Chain-walk audit of 1,487 `sub_adviser` relationships currently DM-routed to the sub-adviser found 8 relationships / 4 sub-advisers / $12.15B AUM with bilateral `wholly_owned`/`parent_brand` chains supporting automatic collapse. Staging changes: 8 `entity_rollup_history(decision_maker_v1)` rows SCD-retargeted (rule `ncen_sub_adviser` → `manual_override`) + 8 `entity_overrides_persistent` rows (action=`merge`, rollup_type=`decision_maker_v1`, identifier_type=`series_id`). Affects: 4 AMG Yacktman funds + 1 AMG Frontier Small Cap + 2 Vaughan Nelson Natixis funds + 1 Calvert EM (MS). validate_entities.py staging = 8 PASS / 2 FAIL / 6 MANUAL (matches prod baseline). promote_staging will also heal the 58 NULL `override_id` rows still deferred in prod (INF22) — expected in-diff noise, collapses to 0 net change after atomic heal.
11b-b. **DM14b — graph-completion workstream** — not yet started. ROADMAP-named candidates (Manulife/Manulife IM $71B, FMR/FIAM $43B, Principal RE $33B, etc.) cannot collapse via chain walk because the entity graph lacks the required `wholly_owned` / `parent_brand` edges. Each requires ADV Schedule A / S corroboration per entity before adding. Missing edges catalogued:
    ...
    Aggregate DM14b scope: 7 edge additions, **123 funds, ~$193.94B AUM**. Once graph is complete, Layer 1 chain walk picks them up automatically — no new code required.
11c. **DM15 — external sub-adviser coverage** — partially addressed. Top-10 divergences reviewed during DM13 scope inspection; external sub-adviser routings (Hartford→Wellington, GS→GQG, Putnam→Franklin, MFC→Boston Partners, Jackson→Mellon) confirmed as legitimate decision-maker chains. L5 batches 1-6 (scoped 2026-04-10) still pending implementation. ~300-500 series, ~$549.7B AUM.
...
20. **`fetch_market.py` completing** — launched 2026-04-16 (PID 78767, nohup). As of last check: batch 5/81, ETA ~85 min. impact_id PK collision fixed in commit `11a35e9` via `_next_id` in manifest.py. **After completion**: run `python3 scripts/enrich_holdings.py --fund-holdings` to refresh `holdings_v2.market_value_live` / `pct_of_float` from updated prices, then `python3 scripts/build_summaries.py --rebuild` to regenerate derived tables, then `make freshness` to confirm `market_data` STALE clears to OK. Check `logs/fetch_market_20260416b.log` for final status.

### Infrastructure follow-ups

22. **INF: `entity_overrides_persistent` sequence migration (migration 006).** Schema has no DEFAULT / sequence on `override_id`; rows inserted without explicit override_id get NULL PK. `_heal_override_ids()` in `promote_staging.py` (`bb444d7`) is a runtime hotfix; the schema itself needs a proper migration to add `DEFAULT nextval('override_id_seq')` + `NOT NULL` constraint. Medium priority — the hotfix handles current traffic.
...
21. ~~**BL-8 — re-enable suppressed pre-commit rules**~~ **Partially DONE 2026-04-16 extended.** Summary of cumulative BL-8 progress:
    ...
    - **Category (b) fixed** (inline fixes, then globally enforced): ruff F541 (`4590887`, 9→0); pylint W0621 (`d59b6fb`+`d629639`, 6→0 globally enforced); pylint W0622 (`ad9775b`, 4 intentional shadows inline-suppressed with permanent-reason comment, then globally enforced). 3 rules.
    - **Category (b) deferred** (1-9 violations each, not yet fixed): pylint W0108 (2), W0212 (3), W0603 (8), W0640 (1), W1309 (9), W1510 (7), I1101 (2), E0606 (2), E0401 (2 — pre-commit env lacks pydantic/curl_cffi); bandit B101 (4), B108 (1), B310 (1), B404 (9), B607 (7). 14 rules.
    - **Category (c) blocked** (>10 violations; require broader cleanup): ruff E501 (908), E402 (21), E701 (17), F841 (17); pylint C/R groups, W0612 (31), W0613 (19), W0718 (163), W1514 (28); bandit B110 (21), B603 (11), B608 (239). 11+ rule groups.
    - **Current pre-commit disable list**: ruff `E501,E402,E701,F841`; pylint `C,R,W0108,W0212,W0603,W0612,W0613,W0640,W0718,W1309,W1510,W1514,I1101,E0401,E0606`.
    - **Inline suppressions**: ~245 total (`# noqa` ×87 — mostly E402; `# pylint: disable` ×77 — mostly broad-except + 4 new redefined-builtin; `# nosec` ×85 — mostly B608).
