...
_Last updated: 2026-04-19 (Session close #12 — doc-sync post-INF39). Main HEAD: `f22312e` (post `block/schema-diff-inf39` merge). **INF39 CLEARED** (`f22312e`) — pre-flight schema-parity gate (`scripts/pipeline/validate_schema_parity.py` + `config/schema_parity_accept.yaml` + `make schema-parity-check`) now live across L3 canonical tables. Rebuild log + findings in `docs/BLOCK_SCHEMA_DIFF_FINDINGS.md` and `docs/SCHEMA_DIFF_PHASE_1_REBUILD_LOG.md`. **Staging backup retention note:** `data/13f_staging.duckdb.pre_inf39_backup` kept through merge + 7 days (target delete date ~2026-04-26). Do not delete before verifying no follow-up rebuild is needed; after 2026-04-26 it is safe to remove. **Hardening trio remaining:** INF40 (stable L3 surrogate row-ID for rollback replay), INF41 (mechanically exhaustive rename sweep discipline), INF42 (derived-artifact / React-dist / CI-fixture hygiene). All three surfaced by the pct-of-so workstream (merged `8925347` + follow-on `12e172b`); tracked in `docs/DEFERRED_FOLLOWUPS.md` and `ROADMAP.md ### Open items`. **validate_entities baseline now corrected to 8 PASS / 2 FAIL / 6 MANUAL** — pre-existing FAILs are `wellington_sub_advisory` + `phase3_resolution_rate` (miscounted as 1 FAIL in headers prior to 2026-04-19). **Terminology:** `pct_of_float` retired project-wide; `pct_of_so` canonical (`holdings_v2.pct_of_so` + `holdings_v2.pct_of_so_source` audit column). **Doc-sync commits this session** (branch `doc/sync-post-schema-diff`): ROADMAP.md, canonical_ddl.md, data_layers.md, pipeline_inventory.md, pipeline_violations.md, REWRITE_BUILD_SHARES_HISTORY_FINDINGS.md, DEFERRED_FOLLOWUPS.md, and this file. **Sibling schema-diff extensions captured** as INF45 (L4 + `entity_current` VIEW), INF46 (L0 control-plane), INF47 (CI wiring — BLOCK-CI-HYGIENE candidate combining INF33/INF42/INF47). Prior session #11 header preserved below._
