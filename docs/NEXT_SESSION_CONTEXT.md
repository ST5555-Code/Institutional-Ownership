...
21. ~~**BL-8 — re-enable suppressed pre-commit rules**~~ **Partially DONE 2026-04-16 extended.** Summary of cumulative BL-8 progress:
    ...
    - **Still open after 2026-04-17 session** (investigated but not committed — session closed before fixes landed):
        - **W0212 (protected-access, 3 violations)** — all in test / instrumentation code that reads `_private` attrs intentionally. Plan: inline `# pylint: disable=protected-access` with permanent-reason comments, then re-enable globally (same pattern as W0622).
        - **W0603 (global-statement, 8 violations)** — module-level singleton patterns (mostly cache / connection accessors). Plan: evaluate per-site — replace with explicit class or inline-suppress if the global is genuinely the right model.
        - **W0613 (unused-argument, 19 violations)** — mostly Flask handler signatures that declare params for routing but don't use them, plus test fixtures. Plan: prefix with `_` or inline-suppress; then consider re-enable. This one straddles (b)/(c) — 19 is borderline but most are mechanical.
