...
_Last updated: 2026-04-13 (Phase 0-B1 design doc + data_freshness pipeline hooks + FreshnessBadge landed. HEAD: 2892009 — docs commit will advance the ref)_
...
- **Branch:** `main`
- **HEAD:** `2892009` (code) / docs commit will be latest
...
### ⭐ Recommended next Claude Code task — Phase 0-B2 smoke CI implementation
...
component piloting in FlowAnalysisTab. Follow-up: wire badge into the
remaining tabs._

From `ARCHITECTURE_REVIEW.md` Phase 0-B2 + `docs/ci_fixture_design.md` acceptance criteria. **Medium risk, ~half day.**

1. **`scripts/build_fixture.py`** — idempotent, reads prod DB read-only, picks reference tickers (AAPL, MSFT, EQT, NVDA), walks the full support graph (holdings → entities → rollups → N-PORT children → fund_family_patterns), writes `tests/fixtures/13f_fixture.duckdb` < 1 MB.
2. **`.github/workflows/smoke.yml`** — copies fixture into place, runs Flask against it, exercises 4 smoke endpoints (`/api/tickers`, `/api/query1`, `/api/entity_graph`, `/api/summary`), asserts HTTP 200 + non-empty body.
3. **`tests/fixtures/responses/*.json`** — one snapshot per endpoint, regenerated by a `--update` flag, not by the test itself on failure.
4. **`test_smoke_response_equality()`** — top-level keys present, row count ±5%, ≥ 1 sentinel value per endpoint.

**Gate:** Phase 0-B2 green unblocks Batch 4-A (Blueprint split).

**Follow-up task (if Phase 0-B2 not prioritized):** wire `FreshnessBadge` into the other 10 tabs. Mapping: Register → `summary_by_parent`, Conviction → `summary_by_parent`, Ownership Trend → `investor_flows`, Fund Portfolio → `fund_holdings_v2`, Sector Rotation → `investor_flows`, etc.
