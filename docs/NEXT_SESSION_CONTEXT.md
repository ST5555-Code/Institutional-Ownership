...
_Last updated: 2026-04-23 (post `inf40-fix` — `sync_staging.py` DDL-first rewrite, CTAS constraint-strip gap closed). Main HEAD at branch point: `6c14c35`. `inf40-fix` branch pending PR._

**inf40-fix closed this session.** `scripts/sync_staging.py` rewritten from `CREATE TABLE AS SELECT` to a DDL-first path that preserves prod's `DEFAULT(nextval('...'))` / `NOT NULL` / sequence references on every staging rebuild. Prod↔staging DDL now byte-identical across all 9 entity tables. Sequence priming moved ahead of CREATE TABLE because DuckDB refuses `CREATE OR REPLACE SEQUENCE` once a dependent DEFAULT clause exists. Verified via sandbox sync + NOT NULL-rejection test + auto-DEFAULT-assignment test. `build_entities.py --reset` dry-run on the sandbox now progresses past the previously-blocked constraint layer and fails on a **separate** issue: `step3_populate_identifiers` hits `Binder Error: no UNIQUE/PRIMARY KEY constraints…` because prod entity tables themselves lack PK/UNIQUE to back `INSERT ... ON CONFLICT DO NOTHING` inference. That is a prod-schema-degradation gap, out of scope for INF40; file as a fresh follow-up if `--reset` needs to be fully green. **Heads-up:** ROADMAP already had one INF40 (`BLOCK-L3-SURROGATE-ROW-ID`, closed 2026-04-22). This is a second ticket that happened to reuse the label — the closed-items log disambiguates via "INF40 (entity-CTAS)".

**Prior session preserved: entity-curation-w1.** Batch-closed two of three standing curation items:
...
- **INF9 closed 2026-04-23** (`inf9-persist`, PR #120, snapshot `20260423_080406`). 11 DM12 `merge`/`decision_maker_v1` overrides (HC Capital Trust 6 + CRI 5, IDs 246–256) promoted to prod; `entity_overrides_persistent` 245 → 256; validate baseline preserved (8 PASS / 1 FAIL wellington / 7 MANUAL). Follow-up **INF9f** (merge-target identifier_type schema gap; 1 residual row S000029852 → Agincourt eid 19021, CRD-only target) still open. Findings doc: `docs/findings/inf9-closure.md`.
...
| `entity_overrides_persistent` | 256 (+11 from INF9 closure 2026-04-23) | `override_id` sequence + NOT NULL (migration 006) |
