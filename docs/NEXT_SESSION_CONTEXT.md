...
20. **`fetch_market.py` still running** — launched 2026-04-16 (PID 78767, nohup). Last check at doc-close: batch ~37/81 (~45% done), ETA ~50 more min. impact_id PK collision fixed in commit `11a35e9` via `_next_id` in manifest.py. **After completion**: run `python3 scripts/enrich_holdings.py --fund-holdings` to refresh `holdings_v2.market_value_live` / `pct_of_float` from updated prices, then `python3 scripts/build_summaries.py --rebuild` to regenerate derived tables with fresh prices, then `make freshness` to confirm `market_data` STALE clears to OK. Check `logs/fetch_market_20260416b.log` for final status.

### Infrastructure follow-ups

22. **INF: `entity_overrides_persistent` sequence migration (migration 006).** Prod override_id backfill ~~**DONE 2026-04-16**~~ via DM14 promote side-effect (`d684e4e`) — 58 NULL rows healed to IDs 25-82, prod table now fully keyed (MAX=90, 0 NULL). **Remaining work:** the schema itself still has no DEFAULT / sequence on `override_id`; new inserts via `admin_bp.py` compute MAX+1 inline (hotfix from `bb444d7`). Migration 006 still pending to add `DEFAULT nextval('override_id_seq')` + `NOT NULL` constraint at the schema level, so future callers don't need the MAX+1 dance. Medium priority — hotfix continues to work.
