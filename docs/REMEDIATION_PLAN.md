...
_Generated: 2026-04-20. Last updated: 2026-04-21 (conv-04)._
...
| int-10 | INF26 OpenFIGI `_update_error()` permanent-pending bug | — | INF26 | DOC_UPDATE_PROPOSAL §Item 2 | CLOSED (PRs #42, #44; staging sweep pending) | `scripts/run_openfigi_retry.py`, `scripts/oneoff/int_10_sweep.py` | None | 1-A | Code fix shipped; staging sweep gated behind `--confirm`. |
...
| obs-04 | MAJOR-8 D-06 13D/G ingestion_impacts grain backfill | §3.1 MAJOR-8 | — | SYSTEM_AUDIT §3.1 | CLOSED (PRs #36, #38; data op pending) | `scripts/oneoff/backfill_13dg_impacts.py` (new) | obs-03 | 2-B | One-off backfill script shipped; data op gated behind `--confirm`. |
...
| obs-08 | MINOR-16 O-05 backup-gap investigation | §8.1 | — | SYSTEM_AUDIT §8.1 | CLOSED (PR #TBD) | `scripts/backup_db.py` + `Makefile` + `MAINTENANCE.md` | None | 2-D | Infra already in place (script + `backup-db` target + `quarterly-update` step 8 + 12 snapshots Apr-10→Apr-19). Apr-14 size drop = `positions` table drop (`d50b602`); 3d gap = dev-only window. MAINTENANCE.md wording fix (manual vs `quarterly-update`) + retention note; see `docs/findings/obs-08-p1-findings.md`. |
...
| mig-13 | pipeline-violations REWRITE tail — residual scope: build_entities, merge_staging | §4.1 | — | pipeline_violations.md:122-485 | OPEN (narrowed) | `scripts/build_entities.py`, `scripts/merge_staging.py` | per-item: some depend on obs/int items | 3-B | `fetch_adv` closed via mig-02 (PR #37); `build_fund_classes` + `build_benchmark_weights` closed via sec-05 (PR #45); 3 resolver scripts (`resolve_agent_names`, `resolve_bo_agents`, `resolve_names`) retired to `scripts/retired/` via sec-06 (PR #48). |
...
| ops-04 | MINOR-9 DOC-04 ARCH_REVIEW vs REACT_MIGRATION contradiction | §7.1 | — | SYSTEM_AUDIT §7.1 Cross-doc 2 | CLOSED (PR #6) | `ARCHITECTURE_REVIEW.md`, `REACT_MIGRATION.md` | None | 5-A | React Phase 4 status aligned. |
| ops-05 | MINOR-10 DOC-05 README_deploy React build prereq | §7.1 | — | SYSTEM_AUDIT §7.1 | CLOSED (PR #6) | `README_deploy.md` | None | 5-A | |
