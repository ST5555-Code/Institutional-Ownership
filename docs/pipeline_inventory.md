...
| `build_fund_classes.py` | local N-PORT XML cache, `fund_classes`, `fund_holdings_v2` | `fund_classes`, `lei_reference`, `fund_holdings_v2.lei` (ALTER+UPDATE) | — (repointed to `fund_holdings_v2` in BLOCK-3 Phase 1, commit `22278b8`) | §1 CHECKPOINT every 5000 only; §5 silent `pass`; §9 hardcoded prod path at `:19`, no `--dry-run` | **REWRITE** (legacy refs cleared; remaining items are framework-level) |
...
| `build_benchmark_weights.py` | `fund_holdings_v2`, `market_data` | `benchmark_weights` | — (repointed to `fund_holdings_v2` via `latest_rm` CTE in BLOCK-3 Phase 1, commit `22278b8`; `--staging` gate added commit `ec75ca2`) | §1 no CHECKPOINT; §9 no `--dry-run` | **REWRITE** (legacy refs cleared; BLOCK-3 Phase 4 applied to prod 2026-04-18) |
