...
| `entity_identifiers` | L3 | `build_entities.py` + `entity_sync.py` | staging→promote (SCD) | **33,521 rows** (2026-04-16 part 2, +287 = 279 series_id + 4 CIK + 4 CRD from bootstrap_residual_advisers); CIK/CRD/SERIES_ID bridge |
| `entity_relationships` | L3 | `build_entities.py` + `entity_sync.py` | staging→promote (SCD) | **18,105 rows** (2026-04-16 part 2, +279 fund_sponsor edges); graph with `is_primary` parent flag; `last_refreshed_at TIMESTAMP` column live since 2026-04-16 (migration `add_last_refreshed_at.py`, 13,685 / 17,826 = 76.77% backfilled from `created_at`; remaining NULLs fill organically on next N-CEN / ADV refresh) |
...
| `entity_rollup_history` | L3 | `build_entities.py` step 7 | staging→promote (SCD) | **55,708 rows** (2026-04-16 part 2, +570 = 285 entities × 2 worldviews); both rollup_type worldviews × ~24.6K entities ≈ 49.3K active, plus closed history rows |
...
| `pending_entity_resolution` | L0 | `scripts/pipeline/shared.entity_gate_check()` + `validate_nport_subset.py` + `resolve_pending_series.py` | direct_write | **5,946 rows** (2026-04-16 part 2) — 4,141 flagged `resolved` (prior sessions via `resolve_pending_series.py` 4-tier T1/T2/T3/S1 + ETF brand seeding) / 1,805 still `pending` status; however **279 of the 1,802 NPORT-pending rows are now entity-backed in prod** (Tier A+B resolver 2026-04-16 part 2) — their status flip to `resolved` happens out-of-band. True unresolvable residual: 337 Tier C series (per-family research needed) + 1,186 deferred synthetics (D13) + 3 13DG entries. Separate follow-up: 2,591 13D/G-only filer CIKs in `beneficial_ownership_v2` outside the MDM entirely |
